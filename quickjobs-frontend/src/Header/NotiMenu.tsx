    ...
    return <Menu shadow="md" width={400} opened={opened} onChange={setOpened}>
        ...
        <Menu.Dropdown onChange={() => setOpened(true)}>
            <div className="flex flex-col gap-1">
                {
                    notifications.map((noti:any, index:number)=> <Notification key={index} onClose={()=>unread(index)} className="hover:bg-cloud-burst-900 cursor-pointer" icon={<IconCheck style={{width: rem(20), height: rem(20) }}/>} color="teal" title={noti.action} mt="md">{noti.message}</Notification>)
                }
                {
